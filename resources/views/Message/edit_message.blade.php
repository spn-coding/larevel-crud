    ...
    <div class="h-[83vh] grid grid-cols-8 gap-4">
        <div class="bg-white rounded-lg col-start-3 col-span-4 row-span-2 px-4 py-2 dark:bg-gray-600 duration-300">
            <div class="relative overflow-x-auto shadow-md dark:shadow-none sm:rounded-lg">
                <form action="/message/{{ $message->id }}" method="POST" class="p-4">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label for="message" class="block text-gray-700 dark:text-gray-300">Message</label>
                        <input type="text" id="" name="message" value="{{ $message->message }}" class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none dark:bg-gray-800 dark:text-gray-300">
                        @error('message')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="time" id="time" name="time" value="{{ $message->time }}" class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none dark:bg-gray-800 dark:text-gray-300">
                        @error('time')
                            <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="mb-4">
                        ...
                        <div class="flex items-center">
                            <input type="radio" id="normal" name="type" value="normal" class="mr-2" {{ $message->type === "normal" ? "checked" : "" }}>
                            <label for="normal" class="mr-4 text-gray-700 dark:text-gray-300">Normal</label>
                            <input type="radio" id="vip" name="type" value="vip" class="mr-2" {{ $message->type === "vip" ? "checked" : "" }}>
